...
abstract class DzqController implements RequestHandlerInterface
{
    ...
    public function inPut($name, $checkValid = true)
    {
        ...
        if ($this->isAdminRoute()) {
            DzqLog::inPutLog($name, 'adminLog');
        }elseif ($this->openApiLog) {
            DzqLog::inPutLog($name, 'apiLog');
        }
        return $p;
    }
     ...
     * 异常日志输出
     */
    public function errorLog($errorMessage = '', $msg = '', $data = [])
    {
        DzqLog::errorLog($errorMessage, $msg, $data);
    }
